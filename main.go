...
func main() {
	...
	combo := widget.NewSelect([]string{"delhi", "noida", "faridabad", "gurgaon"}, func(value string) {
		...
		if err != nil {
			...
			weather1, _ := UnmarshalWeather(body1)
			label2.SetText(fmt.Sprintf(weather1.Sys.Country))
			label3.SetText(fmt.Sprintf("%.2f", weather1.Wind.Speed))
			label4.SetText(fmt.Sprintf("%.2f", weather1.Main.Temp))
			label5.SetText(fmt.Sprintf("%d", weather1.Main.Humidity))
		}
	})
